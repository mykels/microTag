angular.module('vtortola.ng-terminal', [])
	...
	.controller('terminalController', ['$scope', '$timeout', 'terminalConfiguration', 'promptCreator', function ($scope, $timeout, terminalConfiguration, promptCreator) {
		...
		$scope.$on('terminal-command', function (e, cmd) {
			if (cmd.command === 'clear') {
				...
				lastResult.text = [lastResult.text[0].substr(0, lastResult.text[0].length - 1)];
				$scope.results.splice($scope.results.length - 1, $scope.results.length);
	...
	.directive('terminal', function ($document, $interval) {
		return {
			restrict: 'E',
			controller: 'terminalController',
			transclude: true,
			replace: true,
			template: "<section class='terminal' id='terminal' ng-paste='handlePaste($event)'><div class='terminal-viewport' id='terminal-viewport'><div class='terminal-results'></div><span class='terminal-prompt' ng-show='showPrompt'>{{prompt.text}}</span><span class='terminal-input'>{{commandLine}}</span><span class='terminal-cursor'>_</span><input type='text' ng-model='commandLine' class='terminal-target'/></div><div ng-transclude></div></section>",
				...
				return {
					...
					post: function postLink(scope, element, attrs, controller) {
						...
						scope.$watchCollection(function () {
							...
							if (angular.isDefined(oldValues) && oldValues.length && !newValues.length) { // removal detected
							...
							for (var j = 0; j < newValues.length; j++) {
								...
								if (scope.outputDelay) {
									...
									for (i = newValue.text.length - 1; i >= 0; i--) {
