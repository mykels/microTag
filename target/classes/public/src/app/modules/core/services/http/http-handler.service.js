...
function httpHandlerService($q, $http, System, SystemLoader, ObjectUtils, StringUtils) {
	var self = this;
	...
	this.get = function (relativeUrl, config) {
		return $http.get(System.baseUrl + relativeUrl,
			angular.extend({ignoreLoadingBar: true}, config));
	};
	...
	this.head = function (relativeUrl, config) {
		return $http.head(System.baseUrl + relativeUrl,
			angular.extend({ignoreLoadingBar: true}, config));
	};

	this.post = function (relativeUrl, data, config) {
		return $http.post(System.baseUrl + relativeUrl, data,
			angular.extend({ignoreLoadingBar: true}, config));
	};

	this.put = function (relativeUrl, data, config) {
		return $http.put(System.baseUrl + relativeUrl, data,
			angular.extend({ignoreLoadingBar: true}, config));
	};

	this.delete = function (relativeUrl, config) {
		return $http.delete(System.baseUrl + relativeUrl,
			angular.extend({ignoreLoadingBar: true}, config));
	};

	this.jsonp = function (relativeUrl, config) {
		return $http.jsonp(System.baseUrl + relativeUrl,
			angular.extend({ignoreLoadingBar: true}, config));
	};

	this.patch = function (relativeUrl, data, config) {
		return $http.patch(System.baseUrl + relativeUrl, data,
			angular.extend({ignoreLoadingBar: true}, config));
	};
