...
function pagedGridController($timeout, $scope, $templateCache, uiGridConstants, ObjectUtils, ActionBuilder, StringUtils) {
	...
	function initGridOptions() {
		$scope.gridOptions = {
			...
			data: $scope.entities,
			rowTemplate: '<div grid="grid"><div ng-repeat="(colRenderIndex, col) in colContainer.renderedColumns track by col.colDef.name" class="ui-grid-cell aura-grid-cell" ng-class="{ \'ui-grid-row-header-cell\': col.isRowHeader, \'aura-grid-selected\': row.entity.selected}" ui-grid-cell></div></div>'
		};
	}
	...
	function calculateHeight() {
		...
		return Math.max(110, gridLength * $scope.gridOptions.rowHeight) + 32;
	}
